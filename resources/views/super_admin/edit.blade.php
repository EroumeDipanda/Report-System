            ...
            <div class="col-md-12 grid-margin">
                <div class="container">
                    {{-- <div class="card p-4 shadow-sm pt-5">
                        <h3 class="card-title text-center mb-4">MODIFIER UN ADMINISTRATEUR</h3>
                        <form class="forms-sample" action="{{ route('admins.update') }}" method="POST" enctype="multipart/form-data" style="width: 80%; margin: 0 auto;">
                            @csrf
                            <div class="mb-3">
                    ...
                    <div class="card p-4 shadow-sm pt-5">
                        ...
                        <form class="forms-sample"
                            ...
                            <div class="mb-3">
                                <label for="name" class="form-label">{{ __('messages.staff.form.name') }}</label>
                                <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $user->name ?? '') }}" placeholder="{{ __('messages.staff.form.placeholders.name') }}">
                                @if ($errors->has('name'))
                                    <p class="text-danger">{{ $errors->first('name') }}</p>
                                @endif
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label for="username" class="form-label">{{ __('messages.staff.form.email') }}</label>
                                    <input type="text" class="form-control" id="username" name="username" value="{{ old('username', $user->username ?? '') }}" placeholder="{{ __('messages.staff.form.placeholders.email') }}">
                                    @if ($errors->has('username'))
                                        <p class="text-danger">{{ $errors->first('username') }}</p>
                                    @endif
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="email" class="form-label">Email</label>
                                    <input type="email" class="form-control" id="email" name="email" value="{{ old('email', $user->email ?? '') }}" placeholder="Entrez l'addresse Email">
                                </div>
                            </div>
                                    ...
                                    <div class="row">
                                        @foreach ($schools as $school)
                                            <div class="col-md-3">
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox" id="school{{ $school->id }}" name="schools[]" value="{{ $school->id }}"
                                                           {{ isset($user) && $user->adminSchools->contains($school->id) ? 'checked' : '' }}>
                                                    <label class="form-check-label" for="school{{ $school->id }}">
                                                        {{ $school->name }}
                                                    </label>
                                                </div>
                            ...
                            <div class="row mb-3 d-flex">
                                <div class="col-md-6 mb-1">
                                    <label for="contact" class="form-label">{{ __('messages.staff.form.phone') }}</label>
                                    <input type="tel" class="form-control" id="contact" name="contact" value="{{ old('contact', $user->contact ?? '') }}" placeholder="{{ __('messages.staff.form.placeholders.phone') }}">
                                </div>
                                <div class="col-md-6 mb-1">
                                    <label class="form-label" for="photo">{{ __('messages.staff.form.photo') }}</label>
                                    <input class="form-control" type="file" id="photo" name="photo">
