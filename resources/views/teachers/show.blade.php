...
@section('title', __('messages.teachers.title_show'))
...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h3>{{ __('messages.teachers.view_content_title') }}</h3>

                    <div class="row mt-3 mb-3">
                        <div class="col-12 d-flex flex-column flex-md-row justify-content-between">
                            <!-- File Information -->
                            <div class="col-md-6 mb-3 mb-md-0">
                                <h6>{{ __('messages.teachers.file_name') }}: {{ $upload->file_name }}</h6>
                                <p>{{ __('messages.teachers.uploaded_at') }}
                                    {{ \Carbon\Carbon::parse($upload->uploaded_at)->format('l, F j, Y g:i A') }}</p>
                                @if ($upload->status == 'exported')
                                    <p>{{ __('messages.teachers.exported_at') }} {{ $upload->exported_at }}</p>
                                @endif
                            </div>

                            <!-- Action Buttons -->
                            <div
                                class="col-md-6 d-flex flex-column flex-md-row justify-content-md-end align-items-end text-center text-md-end">
                                @if (Auth::user()->role != 'user')
                                    @if ($upload->status == 'pending')
                                        <form id="process-form" action="{{ route('teachers.process', $upload->id) }}"
                                    ...
                                    @elseif ($upload->status == 'processed')
                                        <a href="{{ route('teachers.viewProcessed', $upload->id) }}"
                                            class="btn btn-success d-inline-flex align-items-center me-2 mb-2 mb-md-0">
                                            Ficher Traite &nbsp;<i class="mdi mdi-folder mdi-24px"></i>
                                        </a>
                                        {{-- &nbsp; --}}
                                        <a href="{{ route('teachers.enseignements', ['fileId' => $upload->id]) }}"
                                            class="btn btn-info d-inline-flex align-items-center mb-2 mb-md-0">
                                            Enseignement &nbsp;<i class="mdi mdi-account-group mdi-24px"></i>
                                        </a>
                                    @endif

                                    @if ($upload->status === 'pending')
                                        <form action="{{ route('teachers.reject', $upload->id) }}" method="POST"
                                    ...
                                    @endif
                                @endif
                                <a href="{{ route('teachers.download', $upload->id) }}"
                                    class="btn btn-primary d-inline-flex align-items-center ms-0 ms-md-2">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="text-center">
                                            {{ __('messages.teachers.no_data_available') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
