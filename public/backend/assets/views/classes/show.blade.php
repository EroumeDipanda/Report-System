                ...
                <div class="card-body">
                    ...
                    <div class="row mt-3 mb-3">
                        <div class="col-12 d-flex flex-column flex-md-row justify-content-between">
                            ...
                            <!-- Action Buttons -->
                            <div class="col-md-6 d-flex flex-column flex-md-row justify-content-md-end align-items-end text-center text-md-end">
                                @if (Auth::user()->role != 'user')
                                    @if ($upload->status == 'pending')
                                        <form id="process-form" action="{{ route('classes.process', $upload->id) }}" method="POST" class="mb-2 mb-md-0">
                                            @csrf
                                            <button type="submit" class="btn btn-success d-inline-flex align-items-center" onclick="showProcessing()">
                                                Traiter &nbsp;<i class="mdi mdi-progress-check mdi-24px"></i>
                                            </button>
                                        </form>
                                    @elseif ($upload->status == 'processed')
                                        <a href="{{ route('classes.viewProcessed', $upload->id) }}" class="btn btn-success d-inline-flex align-items-center mb-2 mb-md-0">
                                            Ficher Traite &nbsp;<i class="mdi mdi-folder mdi-24px"></i>
                                        </a>
                                    @endif

                                    @if ($upload->status === 'pending')
                                        <form action="{{ route('classes.reject', $upload->id) }}" method="POST" class="mb-2 mb-md-0 ms-0 ms-md-2">
                                            @csrf
                                            <input type="hidden" name="status">
                                            <button type="submit" class="btn btn-danger d-inline-flex align-items-center">
                                                {{ __('messages.classes.reject') }} &nbsp;<i class="mdi mdi-close-circle mdi-24px"></i>
                                            </button>
                                        </form>
                                    @endif
                                @endif
                                <a href="{{ route('upload.download', $upload->id) }}" class="btn btn-primary d-inline-flex align-items-center ms-0 ms-md-2">
                                    {{ __('messages.classes.download_button') }} &nbsp;<i class="mdi mdi-download mdi-24px"></i>
                                </a>
                            </div>
                        </div>

                        <!-- Processing Indicator -->
                        <div id="processing-indicator" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; text-align: center; justify-content: center; align-items: center; z-index: 9999; font-size: 1.5rem;">
                            <div style="display: inline-flex; flex-direction: column; align-items: center; justify-content: center; height: 100%;">
                                ...
                                <p style="font-size: 2rem; font-weight: bold; margin-top: 10px;">Processing...</p>
                            </div>
                        </div>
