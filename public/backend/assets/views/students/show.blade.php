...
@section('title', __('messages.students.title_show'))
                            ...
                            <div
                                ...
                                @if (Auth::user()->role != 'user')
                                    @if ($upload->status == 'pending')
                                        <form id="process-form" action="{{ route('students.process', $upload->id) }}"
                                    ...
                                    @elseif ($upload->status == 'processed')
                                        <a href="{{ route('students.viewProcessed', $upload->id) }}"
                                            class="btn btn-success d-inline-flex align-items-center mb-2 mb-md-0">
                                            Fichier Traité &nbsp;<i class="mdi mdi-folder mdi-24px"></i>
                                        </a>
                                        &nbsp;
                                        <a href="{{ route('parents.view', ['fileId' => $upload->id]) }}"
                                            class="btn btn-info d-inline-flex align-items-center mb-2 mb-md-0">
                                            View Parents &nbsp;<i class="mdi mdi-account-group mdi-24px"></i>
                                        </a>
                                    @endif
                                    ...
                                    @if ($upload->status === 'pending')
                                        <form action="{{ route('students.reject', $upload->id) }}" method="POST"
                                ...
                                &nbsp;
                                <a href="{{ route('students.download', $upload->id) }}"
